...
//"message" : "Game Client stats not found"
//}
			if (sendType == "END") {
				...
				while (true) {
					...
					if (httpAuthSend("/lol-end-of-game/v1/tft-eog-stats", matchesData)) {
						...
						data["win"] = rank > 4 ? 0 : 1;
						data["time"] = time;
						jsonBody["y_value"] = (double)health;
						/*	member["id"] = std::to_string(myAccountId);
							member["role"] = "self";
							data["member"].push_back(member);*/

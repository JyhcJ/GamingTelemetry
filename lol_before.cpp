...
void Game_Before::getAndSendInfo(std::string sendType) {
	...
	//std::string rankJsonStr = rankData.dump(0); // 缩进4的话
	...
	//}
	//catch (const nlohmann::json::parse_error& e) {
	//	std::cerr << "response_matches_json 解析失败: " << e.what() << std::endl;
	...
	//LOG_IMMEDIATE("最高 : " + BEFORE_RANK);
	////LOG_IMMEDIATE("最高段位 : " + BEFORE_RANK_H);
